...
class Game:
    def __init__(self):
        ...
        self.score=0
        self.font=pg.font.Font("assets/font.ttf",26)
        ...
        while True:
            ...
            for events in pg.event.get():
                if events.type == pg.QUIT:
                    pg.quit()
                    sys.exit()
                if events.type==pg.KEYDOWN and self.is_game_started:
                    if events.key==pg.K_RETURN:
                        self.is_enter_pressed=True
                        self.bird.update_on=True
                    if events.key==pg.K_SPACE and self.is_enter_pressed :
                        self.bird.flap(float(dt))
